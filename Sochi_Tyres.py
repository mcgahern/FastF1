...
# Enable the cache
# ff1.Cache.enable_cache('path/to/cache/')

# Get rid of some pandas warnings that do not apply at the moment
...
# Calculate RaceLapNumber (LapNumber minus 1 since warmup lap is included in lapNumber)
laps['RaceLapNumber'] = laps['LapNumber'] - 1

# Starting from lap 45 it from started raining
...
for driver in drivers:
    ...
    # Since we want to compare distances, we need to collect telemetry
    # lap-by-lap to reset the distance
...
# Everything thats not interemidiate will be slick (not used right now)
telemetry['Compound'].loc[telemetry['Compound'] != 'INTERMEDIATE'] = 'SLICK'

# We want 25 minisectors
num_minisectors = 25

# Whats the distance of the lap
...
fastest_compound = average_speed.loc[average_speed.groupby(['Lap', 'Minisector'])['Speed'].idxmax()]

# Get rid of the speed column and rename the Compound column
fastest_compound = fastest_compound[['Lap', 'Minisector', 'Compound']].rename(columns={'Compound': 'Fastest_compound'})

# Join the fastest compound per minisector with the full telemetry
telemetry = telemetry.merge(fastest_compound, on=['Lap', 'Minisector'])

# Order the data by distance to make sure matplotlib does not get confused
telemetry = telemetry.sort_values(by=['Distance'])

# Assign integer value to the compound because thats what matplot wants
...
generate_minisector_plot(48, save=False, details=True)
